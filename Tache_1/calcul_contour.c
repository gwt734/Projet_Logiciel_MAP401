...
Liste_Contour calcul_contour(Image I)
{
    ...
    Liste_Contour L_C = creer_liste_Contour();
    for (int j = 1; j <= hauteur_image(I); j++)
    {
        for (int i = 1; i <= largeur_image(I); i++)
        {
            if (get_pixel_image(masque, i, j) == NOIR)
            {
                ...
                while (!revenu_au_depart)
                {
                    ...
                    if (o == Est)
                    {
                        set_pixel_image(masque, p_robot.x + 1, p_robot.y + 1, BLANC);
                    }
                ...
                L = ajouter_element_liste_Point(L, p_robot);
                L_C = ajouter_element_liste_Contour(L_C, L);
            }
        }
    }
    return L_C;
